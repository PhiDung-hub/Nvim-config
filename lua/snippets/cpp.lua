...
ls.add_snippets("cpp", {
  ...
  s({ trig = "coutl", keyword = "std cout with endl", dcsr = "Code snippet for printing to std::cout, with endl involved" }
    , {
  ...
  s({ trig = "vec", regTrig = true, name = "vector", dcsr = "Basic vector" }, {
    t("std::vector<"), i(1, "int"), t("> "), i(2, "vec"), t({ ";" }), i(0),
  }),
  ...
  s({ trig = "mpis", name = "map int string", dcsr = "Hash map of int string" }, {
    t("std::unordered_map<int, std::string> "), i(1, "mp"), t(";"), i(0),
  }),
  s({ trig = "mpsi", name = "map string int", dcsr = "Hash map of string int" }, {
    t("std::unordered_map<std::string, int> "), i(1, "mp"), t(";"), i(0),
  }),
  s({ trig = "mpss", name = "map string string", dcsr = "Hash map of string string" }, {
    t("std::unordered_map<std::string, std::string> "), i(1, "mp"), t(";"), i(0),
  }),
  ...
  s({ trig = "tree", name = "starter binary tree", dscr = "Starter code for basic cpp binary tree" }, {
    ...
    t("\tTreeNode(): val("), i(2, "0"), t({ "), left{nullptr}, right{nullptr} {}", "" }),
    t("\tTreeNode("), f(tI, 1), t({ " x) : val(x), left{nullptr}, right{nullptr} {}", "" }),
    t({ "\tTreeNode(" }), f(tI, 1),
    t({ " x, TreeNode *left, TreeNode *right): val(x), left{left}, right{right} {}", "};", "" }),
  }),
  s({ trig = "ptree", name = "print tree", dscr = "print the binary tree" }, {
    t("void printTree(TreeNode *root, const std::string& prefix=\"\", bool isLeft=false) { if(root) { std::cout << prefix; std::cout << (isLeft ? \"├──\" : \"└──\"); std::cout << root->val << std::endl; printTree(root->left,prefix + (isLeft?\"│  \": \"   \"), true); printTree(root->right,prefix + (isLeft?\"│  \": \"   \"), false); } }"),
  }),
  ...
  s({ trig = "dijkstra", name = "dijkstra algorithm",
    dcsr = "Dijkstra algorithm on directed weighted graph, graph edges should be positive" }, {
    t({ "std::vector<pair<int, int>> Dijkstra(int source) {",
